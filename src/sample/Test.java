...
public class Test {
	...
	public static String testCanGetGpsData() {
		...
		Hedwig.GhiroBundle ghiroBundle;

		ArrayList<Hedwig.GhiroBundle> ghiroBundleList = new ArrayList<>();
		ghiroBundleList.add(new Hedwig.GhiroBundle("name", "GPS Test - " + Long.toString(System.currentTimeMillis())));
		ghiroBundleList.add(new Hedwig.GhiroBundle("description", "This is a description"));
		ghiroBundleList.add(new Hedwig.GhiroBundle("image", "/home/joshy/tmp/img/gpstestimg.jpg"));

		try {
			resultJsonString = Hedwig.analyse(Hedwig.Route.NEW_ANALYSIS, ghiroBundleList);
